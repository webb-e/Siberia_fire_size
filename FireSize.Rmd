...
cor(sqrt(alldat$BA),log(alldat$num_fires)); plot(sqrt(alldat$BA)~log(alldat$num_fires))
cor(sqrt(alldat$BA),alldat$mean_size); plot(sqrt(alldat$BA)~alldat$mean_size)
cor(log(alldat$num_fires),alldat$mean_size); plot(log(alldat$num_fires)~alldat$mean_size)
...
data <- firedat %>%   mutate(cluster = as.character(cluster)) %>%
          bind_rows(firedat %>% mutate(cluster = as.character("all"))) %>%
          group_by(FireYr, cluster) %>% summarise(`mean size (ha)` = mean(SizeHa),
                                                         std_size = sd(SizeHa),
                                                  `number of fires` = n(),
                                                  `burned area (ha)` = sum(SizeHa))%>%
          mutate(ste_size = `mean size (ha)`/sqrt(`number of fires`)) %>% drop_na(cluster) %>%
          select(-std_size) %>% 
...
data$cluster<-dplyr::recode(data$cluster, 
                     '2' = 'lowland' ,
                    '1' = 'upland')
data$key<-factor(data$key, levels = c('mean size (ha)',
                                            "number of fires",
                                            "burned area (ha)"))
######
...
base_size = 28
top<-ggplot(data %>% filter(key =='mean size (ha)'), aes(x=FireYr, y = value, color=cluster)) + 
            theme_light(base_size = base_size)+ ylab('mean \nsize (ha)') + xlab('')+
...
middle<-ggplot(data %>% filter(key == "number of fires"), aes(x=FireYr, y = value, color=cluster)) + 
          theme_light(base_size = base_size)+ ylab('number\nof fires') + xlab('')+
          geom_point( size=3) + geom_line(size=1)+
          scale_color_manual(values=c('#756555', '#BC944C', '#a4ab82')) +
...
bottom<-ggplot(data %>% filter(key == "burned area (ha)"), aes(x=FireYr, y = value, color=cluster)) + 
            theme_light(base_size = base_size)+
           ylab('burned \narea (ha)') + xlab('fire year')+
            geom_point( size=3) + geom_line(size=1)+
            scale_color_manual(values=c('#756555', '#BC944C', '#a4ab82')) +
            scale_y_continuous( labels = scales::label_number_si()) +
            theme(panel.grid.major = element_blank(),
...
allplots<-plot_grid(top, middle, bottom, align='v',
                    ncol=1, rel_widths = c(1.2, 1.2, 0.05))
######
### save
#####
ggsave("Fig1.jpeg", allplots,
       path="figures/",
...
Cumulative area burned in each fire year.
...
firedat$FireYr<-as.factor(firedat$FireYr)
firedat$UniqueId<-as.factor(firedat$UniqueId)

df1<-firedat %>% 
  mutate(percentile_rank = ntile(SizeHa, 20)) %>% arrange(percentile_rank)

firedat$fire_group <- ifelse(firedat$SizeHa <10000, 'small (0-10K ha)',
                             ifelse(firedat$SizeHa<100000, 'medium (10-100K ha)',
                                    ifelse(firedat$SizeHa<1000000, 'large (100K-1M ha)', 'mega (>1M ha)')))

firedat$fire_group<-factor(firedat$fire_group, levels=rev(c('small (0-10K ha)','medium (10-100K ha)', 'large (100K-1M ha)', 'mega (>1M ha)')))
firedat <- firedat %>% arrange(fire_group)

fire_count <- firedat %>% group_by(FireYr, fire_group) %>% summarise(num_fires = n(), totalburn = sum(SizeHa)) %>% arrange(fire_group)
...
plot<- ggplot(firedat, aes(x=FireYr, y=SizeHa, fill=fire_group)) +
        geom_col() + 
  scale_fill_manual(name='Fire size',
                    values = rev(c("#E5E5E5", "#BFCDD6", "#78A4B0", "#4A7C88")))+
        scale_y_continuous(  labels = scales::label_number(scale_cut=cut_si('')),
                             name="Cumulative area burned (ha)") +
        theme_light(base_size = 23) + xlab('Fire year') +
        theme(axis.text.x = element_text(angle = 45,  hjust=1),
              axis.title.x = element_text(vjust=-0.7),
              legend.position=c(.2,.85),
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank()) +
        geom_text(data=fire_count, aes(label = num_fires, x=FireYr, y=totalburn),fontface='bold',
                  size = 5,  position = position_stack(vjust = .5))


ggsave("figure3.jpeg", plot,
       path="figures/",
...
fire_stats<- firedat %>% group_by(fire_group) %>% 
              summarise(num_fires = n(), totalburn = sum(SizeHa)) %>%
              mutate(percent_fires = 100*num_fires/(sum(num_fires)),
                     percent_burned = 100*totalburn/sum(totalburn))
```
## Figure 4
...
## break up size classes
firedat$fire_group <- ifelse(firedat$SizeHa <10000, 'small',
                             ifelse(firedat$SizeHa<100000, 'medium',
                                    ifelse(firedat$SizeHa<1000000, 'large', 'mega')))
...
alldat$fire_group<-factor(alldat$fire_group, levels=c('small',
                                                          'medium', 
                                                          'large',
                                                          'mega'))
...
TOP<- ggplot(alldat %>% drop_na(cluster) %>% filter(variable == "precip"),
  ...
  geom_bar(position=position_dodge2(width=0.5, preserve = 'single'),width=0.5, stat='identity') + 
  ...
  scale_y_continuous(labels = scales::label_number_si(), limits=c(0,20000000))+
...
BOTTOM<- ggplot(alldat %>% drop_na(cluster) %>% filter(variable == "temp"),
  ...
  geom_bar(position=position_dodge2(width=0.5, preserve = 'single'),width=0.5, stat='identity') + 
  facet_grid(variable~cluster, switch = "y", labeller =label_names)+
  xlab('Fire size class ') + ylab('burn area (ha)') +
  scale_fill_manual(values=c( "#355070", "#e56b6f"),
